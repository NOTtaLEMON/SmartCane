/*
 * ============================================================================
 *  SMART CANE CLIP-ON  |  Car Detector (YOLOv8n TFLite)
 * ============================================================================
 *  Loads car_float16.tflite — a YOLOv8n detection model trained on car
 *  images, nc=1 (single class: car).
 *  Output tensor: [1, 5, 2100]  (4 box coords + 1 class score × 2100 anchors).
 *
 *  ASSET REQUIRED
 *  --------------
 *  app/src/main/assets/car_float16.tflite
 * ============================================================================
 */
...
class CarDetector(context: Context) : AutoCloseable {

    companion object {
        private const val MODEL_FILENAME = "car_float16.tflite"
        private const val INPUT_SIZE     = 320
        private const val CONF_THRESHOLD = 0.75f
        private const val IOU_THRESHOLD  = 0.45f
        private const val NUM_COORDS     = 4

        private val labels = arrayOf("car")
    }
    ...
    fun detect(bitmap: Bitmap): List<DetectionResult> {
        ...
        for (a in 0 until numAnch) {
            ...
            candidates.add(
                DetectionResult(
                    label       = labels.getOrElse(bestClass) { "car" },
    ...
    private fun iou(a: RectF, b: RectF): Float {
        val interX1 = max(a.left, b.left);  val interY1 = max(a.top, b.top)
        val interX2 = min(a.right, b.right); val interY2 = min(a.bottom, b.bottom)
