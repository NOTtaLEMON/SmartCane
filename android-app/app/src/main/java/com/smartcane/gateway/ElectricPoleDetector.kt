/*
 * ============================================================================
 *  SMART CANE CLIP-ON  |  Electric Pole Detector (YOLOv8n TFLite)
 * ============================================================================
 *  Loads electric_pole_float16.tflite — a YOLOv8n detection model trained
 *  on electric pole images, nc=1 (single class: electric_pole).
 *  Output tensor: [1, 5, 2100]  (4 box coords + 1 class score × 2100 anchors).
 *
 *  ASSET REQUIRED
 *  --------------
 *  app/src/main/assets/electric_pole_float16.tflite
 * ============================================================================
 */
...
class ElectricPoleDetector(context: Context) : AutoCloseable {

    companion object {
        private const val MODEL_FILENAME = "electric_pole_float16.tflite"
        private const val INPUT_SIZE     = 320
        private const val CONF_THRESHOLD = 0.70f
        private const val IOU_THRESHOLD  = 0.45f
        private const val NUM_COORDS     = 4

        private val labels = arrayOf("electric pole")
    }
    ...
    fun detect(bitmap: Bitmap): List<DetectionResult> {
        ...
        for (a in 0 until numAnch) {
            ...
            candidates.add(
                DetectionResult(
                    label       = labels.getOrElse(bestClass) { "electric pole" },
